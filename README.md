...
| 날짜 | 산출물 | 설명 |
|---|---|---|
| 2026-08-26 | [moe-prefill-quantum](./moe-prefill-quantum/) | MoE 추론 서버에서 --mixed-prefill-quantum 기본값 128 이 prefill 을 절반으로 묶는 이유(전문가당 3토큰). 세션이 겹칠 때만 적용되고 컨텍스트가 길수록 효과가 준다(2만 2.31배, 17.7만 1.66배) |
...
| 브랜치 | 역할 |
|---|---|
| `main` | 콘텐츠 원본. 홈랩 git-sync 가 이걸 당겨서 서빙한다 |
| `gh-pages` | 리다이렉트 전용. 옛 주소 `nacyot.github.io/artifacts/*` 를 새 도메인으로 301 시킨다 |

`gh-pages` 에는 글이 없다. `CNAME` 파일 하나로 GitHub Pages 의 서버측 301 을 켜고,
그게 안 먹는 경우를 대비한 meta refresh 스텁과 404.html 만 들어 있다.
새 글을 올릴 때 `gh-pages` 도 갱신해야 한다(슬러그별 스텁 추가). 실제로는 301 이 먼저 잡아서
스텁까지 갈 일은 없지만, 스텁이 없어도 404.html 이 경로를 살려 넘긴다.
